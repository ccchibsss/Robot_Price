...
+ ПОДДЕРЖКА ПУБЛИЧНОГО ДОСТУПА К GOOGLE SHEETS
+ ВИЗУАЛЬНАЯ НАСТРОЙКА КОЛОНОК
ВЕРСИЯ 13.0 - ПОЛНЫЙ ФУНКЦИОНАЛ С ОБУЧЕНИЕМ И ПУБЛИЧНЫМ ДОСТУПОМ
"""
...
class Config:
    ...
    gs_mapping: Dict[str, str] = field(default_factory=lambda: {
        'sku': 'артикул',
        'price': 'цена_базовая',
        'retail_price': 'цена_розница',
        'stock': 'остаток',
        'brand': 'бренд',
        'name': 'название',
        'category': 'категория',
    ...
    def _migrate_config_v1_to_v2(cls, config_dict: Dict) -> Dict:
        ...
        config_dict.setdefault('use_public_access', True)
        config_dict.setdefault('gs_mapping', {
            'sku': 'артикул',
            'price': 'цена_базовая',
            'retail_price': 'цена_розница',
            'stock': 'остаток',
    ...
    def _init_log_file(self) -> None:
        ...
        try:
            ...
            with open(self.log_path, 'a', encoding='utf-8') as f:
                ...
                f.write(f"Версия: 13.0 | Платформа: {platform.platform()}\n")
                f.write(f"{'='*60}\n")
        except Exception as e:
...
class GoogleSheetsDatabase:
    ...
    # Системные имена полей -> их описание
    FIELD_MAPPING = {
        'sku': {'label': 'Артикул', 'required': True, 'type': 'string'},
        'price': {'label': 'Цена базовая', 'required': True, 'type': 'float'},
        'retail_price': {'label': 'Цена розница', 'required': False, 'type': 'float'},
        'stock': {'label': 'Остаток', 'required': False, 'type': 'int'},
        'brand': {'label': 'Бренд', 'required': False, 'type': 'string'},
        'name': {'label': 'Название', 'required': False, 'type': 'string'},
        'category': {'label': 'Категория', 'required': False, 'type': 'string'},
        'description': {'label': 'Описание', 'required': False, 'type': 'string'},
        'weight': {'label': 'Вес', 'required': False, 'type': 'float'},
        'barcode': {'label': 'Штрихкод', 'required': False, 'type': 'string'},
        'country': {'label': 'Страна', 'required': False, 'type': 'string'},
        'warranty': {'label': 'Гарантия', 'required': False, 'type': 'string'},
        'dimensions': {'label': 'Размеры', 'required': False, 'type': 'string'},
        'currency': {'label': 'Валюта', 'required': False, 'type': 'string'},
        'supplier': {'label': 'Поставщик', 'required': False, 'type': 'string'},
        'last_update': {'label': 'Дата обновления', 'required': False, 'type': 'string'}
    }
    ...
    def __init__(self, config: Config, logger: StreamlitLogger):
        ...
        self.cache_ttl = 300
        self._gs_columns: Dict[str, str] = {}
        self.use_public_access = config.use_public_access
    ...
    def _connect(self) -> None:
        """Подключение к Google Sheets через публичный доступ или сервисный аккаунт"""
        try:
    ...
    def _detect_column_mapping(self) -> None:
        """Автоматическое определение колонок в Google Sheets"""
        if self.df is None or self.df.empty:
            self._gs_columns = self._get_default_mapping()
            return
        
        columns = self.df.columns.tolist()
        mapping = {}
        
        # Пытаемся определить каждое поле
        for field, info in self.FIELD_MAPPING.items():
            found = False
            
            # 1. Проверяем точное совпадение
            if info['label'] in columns:
                mapping[field] = info['label']
                found = True
            
            # 2. Проверяем по синонимам
            if not found:
                synonyms = self._get_synonyms(field)
                for col in columns:
                    col_lower = col.lower().strip()
                    for syn in synonyms:
                        if syn.lower() in col_lower or col_lower in syn.lower():
                            mapping[field] = col
                            found = True
                            break
                    if found:
            ...
            # 3. Если не найдено - используем стандартное имя
            if not found:
                mapping[field] = info['label']
        
        self._gs_columns = mapping
        self.logger.info(f"Определен маппинг колонок Google Sheets: {mapping}")
    
    def _get_synonyms(self, field: str) -> List[str]:
        """Получение синонимов для поля"""
        synonyms = {
            'sku': ['артикул', 'код', 'id', 'sku', 'article', 'art', 'номер'],
            'price': ['цена', 'cost', 'прайс', 'стоимость', 'цена_базовая'],
            'retail_price': ['цена_розница', 'розница', 'retail', 'цена_продажи'],
            'stock': ['остаток', 'количество', 'кол-во', 'stock', 'quantity', 'наличие'],
            'brand': ['бренд', 'производитель', 'manufacturer', 'vendor', 'марка'],
            'name': ['название', 'наименование', 'product', 'title', 'имя'],
            'category': ['категория', 'раздел', 'группа', 'category', 'section', 'тип'],
            'description': ['описание', 'desc', 'характеристики', 'description'],
            'weight': ['вес', 'масса', 'weight', 'mass'],
            'barcode': ['штрихкод', 'ean', 'upc', 'barcode', 'баркод'],
            'country': ['страна', 'производство', 'origin', 'country', 'made_in'],
            'warranty': ['гарантия', 'guarantee', 'warranty', 'срок_гарантии'],
            'dimensions': ['размеры', 'габариты', 'dimensions', 'size', 'размер'],
            'currency': ['валюта', 'currency', 'valuta'],
            'supplier': ['поставщик', 'supplier', 'vendor', 'продавец'],
            'last_update': ['дата_обновления', 'updated', 'last_update', 'обновлено']
        }
        return synonyms.get(field, [])
    
    def _get_default_mapping(self) -> Dict[str, str]:
        """Получение маппинга по умолчанию"""
        return {field: info['label'] for field, info in self.FIELD_MAPPING.items()}
    
    def get_column(self, field: str) -> str:
        """Получение имени колонки для поля"""
        return self._gs_columns.get(field, self.FIELD_MAPPING.get(field, {}).get('label', field))
    
    def get_sku_column(self) -> str:
        """Получение колонки артикула"""
        return self.get_column('sku')
    
    def get_price_column(self) -> str:
        """Получение колонки цены"""
        return self.get_column('price')
    
    def get_stock_column(self) -> str:
        """Получение колонки остатка"""
        return self.get_column('stock')
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt < 2:
                        self.logger.warning(f"Ошибка API, повторная попытка {attempt+1}/3: {e}")
            ...
            try:
                ...
                self.logger.error(f"Ошибка API при загрузке: {e}")
                # Пробуем загрузить по частям
                all_values = []
                row_count = worksheet.row_count
                for start_row in range(1, min(row_count, 10000), 1000):
                    chunk = worksheet.get_values(f'A{start_row}:Z{start_row+999}')
    ...
    def _validate_and_fix(self) -> None:
        ...
        required_cols = ['артикул', 'цена_базовая']
        ...
        optional_cols = ['бренд', 'название', 'цена_розница', 'остаток', 'категория', 
        ...
        self.df = self.df[self.df['артикул'] != 'NAN']
        self.df = self.df[self.df['артикул'] != 'NONE']
        ...
        # Если розничная цена = 0, устанавливаем с наценкой
    ...
    def _create_default_sheet(self) -> None:
        ...
        try:
            ...
            headers = ['артикул', 'цена_базовая', 'бренд', 'название', 'цена_розница', 
                      'остаток', 'категория', 'описание', 'вес', 'размеры', 'штрихкод', 
                      'страна', 'гарантия', 'дата_обновления', 'источник']
    ...
    def get_product_by_sku(self, sku: str) -> Optional[Dict]:
        ...
        sku_col = self.get_sku_column()
        product = self.df[self.df[sku_col] == str(sku).strip().upper()]
    ...
    def update_product(self, sku: str, **kwargs) -> bool:
        """Обновление товара с использованием маппинга колонок"""
        if self.df is None:
            return False
        
        sku = str(sku).strip().upper()
        
        # Получаем имя колонки для SKU
        sku_col = self.get_sku_column()
        
        # Ищем товар
        mask = self.df[sku_col] == sku
        ...
        if not mask.any():
            ...
            # Заполняем данные с учетом маппинга
            for field, value in kwargs.items():
                col_name = self.get_column(field)
        ...
        # Обновляем дату
        date_col = self.get_column('last_update')
        if date_col in self.df.columns:
            self.df.loc[mask, date_col] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def get_products_for_yandex(self, markup_percent: float = 0, 
        ...
        for _, row in self.df.iterrows():
            try:
                ...
                base_price = float(row.get(price_col, 0))
                ...
                self.logger.error(f"Ошибка подготовки товара {row.get(sku_col, 'unknown')}: {e}")
                continue
    ...
    def get_stats(self) -> Dict:
        ...
        prices = pd.to_numeric(self.df[price_col], errors='coerce')
        base_prices = pd.to_numeric(self.df['цена_базовая'], errors='coerce')
        stocks = pd.to_numeric(self.df[stock_col], errors='coerce').fillna(0)
    ...
    def diagnostic_check(self) -> Dict[str, Any]:
        """Диагностическая проверка подключения к Google Sheets"""
        result = {
            'status': 'unknown',
            'errors': [],
            'warnings': [],
        ...
        try:
            ...
            if os.path.exists(self.config.google_credentials_json):
                result['info']['credentials_file'] = 'found'
                try:
                    with open(self.config.google_credentials_json, 'r') as f:
                        creds_data = json.load(f)
                        required_fields = ['type', 'project_id', 'client_email', 'private_key']
                        missing = [f for f in required_fields if f not in creds_data]
                        if missing:
                            result['warnings'].append(f"Отсутствуют поля в JSON: {missing}")
                except json.JSONDecodeError:
                    result['errors'].append("Некорректный JSON в файле учетных данных")
            else:
                if not self.config.use_public_access:
                    result['warnings'].append(f"Файл {self.config.google_credentials_json} не найден")
            ...
            if self.client:
                try:
                    if self.config.google_sheet_id:
                        sheet = self.client.open_by_key(self.config.google_sheet_id)
                        result['info']['spreadsheet'] = sheet.title
                        result['info']['spreadsheet_id'] = self.config.google_sheet_id
                        
                        try:
                            worksheet = sheet.worksheet(self.config.google_sheet_name)
                            result['info']['worksheet'] = worksheet.title
                            result['info']['rows'] = worksheet.row_count
                            result['info']['cols'] = worksheet.col_count
                        except:
                            result['warnings'].append(f"Лист {self.config.google_sheet_name} не найден")
                    else:
                        result['warnings'].append("ID таблицы не указан")
                    
                    result['status'] = 'connected'
                except gspread.exceptions.SpreadsheetNotFound:
                    result['errors'].append(f"Таблица {self.config.google_sheet_id} не найдена")
                    result['status'] = 'error'
                except gspread.exceptions.APIError as e:
                    result['errors'].append(f"Ошибка API: {e}")
                    result['status'] = 'error'
            else:
...
try:
    ...
    if config.google_sheet_id and config.use_public_access:
        st.session_state.product_db = GoogleSheetsDatabase(config, st.session_state.logger)
    else:
        if os.path.exists(config.google_credentials_json):
            st.session_state.product_db = GoogleSheetsDatabase(config, st.session_state.logger)
        else:
            st.session_state.product_db = None
            st.info("📊 Для работы с Google Sheets настройте подключение во вкладке 'Google Sheets'")
except Exception as e:
    st.session_state.product_db = None
    st.warning(f"⚠️ Не удалось подключиться к Google Sheets: {str(e)}")
    st.info("💡 Перейдите во вкладку 'Google Sheets' для настройки")
...
st.info("💡 Перейдите во вкладку 'Google Sheets' для настройки подключения")
return
...
with settings_tab3:
    ...
    st.text_input("Название листа", key="google_sheet_name", 
                 value=st.session_state.config.google_sheet_name)
    st.checkbox("Использовать публичный доступ", key="use_public_access",
               value=st.session_state.config.use_public_access)
...
if st.button("💾 Сохранить настройки", use_container_width=True, type="primary", key="save_settings_sidebar_btn"):
    ...
    config.use_public_access = st.session_state.use_public_access
...
st.markdown('<div class="main-header">🤖 Робот для загрузки и анализа прайсов v13.0</div>', 
       unsafe_allow_html=True)
...
# ===================================================================
# БЛОК 17: ГЛАВНАЯ ФУНКЦИЯ ПРИЛОЖЕНИЯ
# ===================================================================
...
st.set_page_config(
    page_title="🤖 Робот для прайсов v13.0",
    page_icon="🤖",
    layout="wide",
...
# ===================================================================
# БЛОК 18: ТОЧКА ВХОДА И ЗАПУСК ИЗ КОМАНДНОЙ СТРОКИ
# ===================================================================
