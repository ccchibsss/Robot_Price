...
+ ИНТЕРАКТИВНЫЙ ВЫБОР КОЛОНОК (1 РАЗ НАСТРОЙКА)
...
ВЕРСИЯ 12.0 - МАППИНГ УПРОЩЕН ДО СОХРАНЕНИЯ СООТВЕТСТВИЙ
"""

# ===================================================================
# БЛОК 1: ВСЕ ИМПОРТЫ (полный набор – без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 2: РАСШИРЕННЫЕ ПЕРЕЧИСЛЕНИЯ И СТРУКТУРЫ (без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 3: КОНФИГУРАЦИЯ С ПОСТАВЩИКАМИ (УПРОЩЕННЫЙ МАППИНГ)
# ===================================================================

@dataclass
class SupplierConfig:
    """Конфигурация одного поставщика – теперь маппинг хранится здесь как простой словарь"""
    name: str = ''
    email: str = ''
    email_password: str = ''
    ...
    # Единственное поле для маппинга – словарь {поле: название_колонки_в_файле}
    column_mapping: Dict[str, str] = field(default_factory=dict)
    
    # Правила обработки (без маппинга)
...
# ===================================================================
# БЛОК 4: КОНФИГУРАЦИЯ ПРИЛОЖЕНИЯ (УДАЛЕНЫ ВСЕ МАППИНГ-МЕТОДЫ)
# ===================================================================
...
class Config:
    ...
    # Удалён column_mapping – больше не нужен глобально
    ...
    # Удалены все методы, связанные с маппингом (get_supplier_mapping, set_supplier_mapping, _save_mapping_history, get_mapping_history)
    ...
    def _migrate_config_v1_to_v2(cls, config_dict: Dict) -> Dict:
        ...
        # column_mapping удалён – не переносим
        return config_dict

# ===================================================================
# БЛОК 5: РАСШИРЕННАЯ СИСТЕМА ЛОГГИРОВАНИЯ (без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 6: БАЗА ДАННЫХ SQLITE (удалена таблица mapping_configs)
# ===================================================================
...
class Database:
    ...
    def _migrate_v2_to_v3(self) -> None:
        with self.get_connection() as conn:
            ...
            ''')
            # Таблица mapping_configs больше не создаётся
            conn.commit()
    ...
    def _init_db(self) -> None:
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Таблица mapping_configs удалена
...
# ===================================================================
# БЛОК 7: GOOGLE SHEETS БАЗА (без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 8: КЛИЕНТ ЯНДЕКС МАРКЕТ (без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 9: ЗАГРУЗЧИК ИЗ ПОЧТЫ (без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 10: ПАРСЕР ПРАЙСОВ (УПРОЩЕН – УДАЛЕНЫ ВСЕ МЕТОДЫ МАППИНГА, КРОМЕ _apply_mapping)
# ===================================================================
...
class PriceParser:
    ...
    # Единственный метод маппинга – просто переименовывает колонки
...
# ===================================================================
# БЛОК 11: МНОГОПОСТАВЩИЧНЫЙ ЗАГРУЗЧИК (без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 12: АНАЛИЗАТОР ПРАЙСОВ (без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 13: СИСТЕМА УВЕДОМЛЕНИЙ (без изменений)
# ===================================================================
...
# ===================================================================
# БЛОК 14: ОСНОВНОЙ РОБОТ (УДАЛЕНЫ ВСЕ ССЫЛКИ НА МАППИНГ, КРОМЕ ПОЛУЧЕНИЯ mapping)
# ===================================================================
...
class PriceRobot:
    ...
    def _process_single_file(self, file_data: Dict) -> Dict:
        ...
        try:
            ...
            # Получаем маппинг из конфига поставщика
            supplier_config = self.config.get_supplier_by_name(supplier_name)
            mapping = supplier_config.column_mapping if supplier_config else {}
            processing_rules = supplier_config.processing_rules if supplier_config else None
            custom_markup = supplier_config.get_effective_markup(self.config.markup_percent) if supplier_config else 0
            
            # Если маппинг есть – применяем, иначе парсим с автоопределением
            if mapping and any(mapping.values()):
                ...
                self.logger.info(f"Файл {filename} распарсен с автоопределением колонок (маппинг не найден)")
...
# ===================================================================
# БЛОК 15: ИНТЕРФЕЙС STREAMLIT (ЗАМЕНЕНА ВКЛАДКА МАППИНГА НА ПРОСТОЙ ВЫБОР КОЛОНОК)
# ===================================================================

def render_column_mapping_ui() -> None:
    """Простой интерфейс для настройки соответствия колонок – 1 раз обучить"""
    st.subheader("📋 Настройка колонок прайсов")
    st.markdown("""
    **Загрузите файл прайса и укажите, какая колонка соответствует каждому полю.**
    Программа запомнит эти настройки для данного поставщика и будет применять автоматически при следующих загрузках.
    """)
    
    suppliers = st.session_state.config.suppliers
    supplier_names = [s.get('name', 'Unknown') for s in suppliers]
    if not supplier_names:
        st.warning("⚠️ Сначала создайте поставщика на вкладке 'Поставщики'.")
        return
    
    selected_supplier = st.selectbox("Выберите поставщика", supplier_names, key="mapping_supplier_select")
    ...
    if current_mapping and any(current_mapping.values()):
        st.info(f"✅ Для поставщика **{selected_supplier}** уже настроен маппинг. Вы можете переопределить его, загрузив новый файл.")
        with st.expander("Показать текущий маппинг"):
            st.json(current_mapping)
    
    uploaded_file = st.file_uploader(
        "📤 Загрузите файл прайса для настройки колонок",
        type=['xlsx', 'xls', 'csv', 'xml', 'json', 'txt', 'xlsm', 'ods', 'xlsb'],
        key="column_mapping_file_uploader"
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            df = parser.parse(filename, content)  # парсим без маппинга, чтобы увидеть все колонки
            if df.empty:
                st.error("Файл пуст или не содержит данных.")
                return
            columns = df.columns.tolist()
            st.success(f"Файл загружен, найдено {len(columns)} колонок, {len(df)} строк.")
            st.dataframe(df.head(10), use_container_width=True)
            
            st.markdown("### 🎯 Укажите соответствие колонок")
            st.markdown("Выберите для каждого поля, какая колонка в файле ему соответствует.")
            ...
            for i, (field, label) in enumerate(field_options.items()):
                with col1 if i % 2 == 0 else col2:
                    # Пытаемся найти похожую колонку для авто-подстановки
                    default_index = 0
                    for idx, col in enumerate(col_options):
                        if col and (field in col.lower() or col.lower() in field):
                            default_index = idx
                            break
                    selected_col = st.selectbox(
                        ...
                        options=col_options,
                        index=default_index,
                        key=f"mapping_{field}_{selected_supplier}"
                    )
                    if selected_col:
                        mapping[field] = selected_col
            
            if not mapping.get('sku') or not mapping.get('price'):
                st.warning("⚠️ Поля 'Артикул' и 'Цена' обязательны для заполнения.")
            else:
                st.markdown("---")
                st.subheader("🔍 Предпросмотр результата")
                # Применяем маппинг для показа
                test_df = parser._apply_mapping(df.copy(), mapping)
                st.dataframe(test_df.head(20), use_container_width=True)
                
                col1, col2, col3 = st.columns([2,1,1])
                with col1:
                    comment = st.text_input("Комментарий к сохранению (необязательно)", key="mapping_comment")
                with col2:
                    if st.button("💾 Сохранить маппинг", type="primary", use_container_width=True):
                        if supplier_config:
                            supplier_config.column_mapping = mapping
                            # Обновляем список поставщиков в конфиге
                            ...
                            st.success(f"✅ Маппинг для {selected_supplier} сохранен!")
                            st.balloons()
                            time.sleep(1)
                            st.rerun()
                        else:
                            st.error("Ошибка: поставщик не найден")
                with col3:
                    if st.button("🔄 Сбросить маппинг", use_container_width=True):
                        if supplier_config:
...
def render_supplier_settings() -> None:
    """Интерфейс управления поставщиками (уже есть в коде, добавим ссылку на настройку колонок)"""
    ...
    if suppliers:
        ...
        for i, supplier in enumerate(suppliers):
            with st.expander(
                ...
                with col4:
                    st.link_button("⚙️ Настроить колонки", "?tab=columns", use_container_width=True)
    ...
    with st.expander("➕ Добавить нового поставщика", expanded=not suppliers):
        ...
        if st.button("✅ Добавить поставщика", type="primary", use_container_width=True):
            if not new_name or not new_email or not new_password:
                ...
                st.session_state.config._save_suppliers()
                st.success(f"✅ Поставщик {new_name} добавлен! Теперь перейдите на вкладку 'Настройка колонок' для загрузки первого прайса.")
                st.rerun()

# ===================================================================
# БЛОК 16: ОСТАЛЬНЫЕ ВКЛАДКИ (Dashboard, Аналитика, Товары, Логи) – без изменений, 
# но вкладка Маппинг заменена на render_column_mapping_ui
# ===================================================================
...
# ===================================================================
# БЛОК 17: ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (без изменений)
# ===================================================================
        ...
        with st.spinner("Парсинг файлов..."):
            ...
            for file_data in downloaded_files:
                try:
                    ...
                    if not content:
                        continue
                    supplier_config = config.get_supplier_by_name(supplier_name)
                    mapping = supplier_config.column_mapping if supplier_config else {}
                    if mapping and any(mapping.values()):
...
def render_main_content() -> None:
    ...
    with main_tab3:
        # Заменяем вкладку маппинга на простую настройку колонок
